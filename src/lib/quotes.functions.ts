...
export const fetchQuotes = createServerFn({ method: "POST" })
  ...
  .handler(async ({ data }): Promise<QuoteResult[]> => {
    ...
    const fetchOne = async (ticker: string): Promise<QuoteResult> => {
      ...
      try {
        ...
        if (!resp.ok) {
          ...
          return { ticker, price: null, error: msg };
        }
        ...
        if (!r) return { ticker, price: null, error: "Sem retorno da API" };
        return {
          ticker,
    ...
    // Plano gratuito Brapi permite apenas 1 ativo por requisição.
    // Executa sequencialmente para evitar rate-limit.
