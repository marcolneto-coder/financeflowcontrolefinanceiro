    ...
    <div className="p-8 max-w-4xl">
      <header className="flex justify-between items-end mb-8">
        <div>
          ...
          <h1 className="text-3xl font-semibold tracking-tight">Cartões de Crédito</h1>
        </div>
          ...
          {state.creditCards.map((card) => {
            ...
            return (
              <div key={card.id} className="space-y-4">
                {/* Card visual */}
                <div
                  className="relative aspect-[1.6/1] p-6 rounded-2xl text-white shadow-xl overflow-hidden flex flex-col justify-between"
                  style={{
                ...
                >
                  ...
                  <div className="flex justify-between items-start relative z-10">
                    <div className="size-10 bg-white/20 rounded-lg backdrop-blur-sm" />
                  ...
                  <div className="relative z-10">
                    <p className="text-sm opacity-60 mb-1">{card.name}</p>
                    <p className="text-lg tracking-[0.2em] font-light">
                      •••• •••• •••• {card.lastDigits}
                    </p>
                  </div>
                </div>
                  ...
                  <div className="h-1.5 bg-muted rounded-full overflow-hidden">
                    <div
                      ...
                      style={{
                        ...
                        backgroundColor: pct > 80 ? "var(--expense)" : card.color,
                      }}
                    />
                  </div>
...
function CardFormDialog({
  ...
  editCard: { id: string; name: string; lastDigits: string; limit: number; closingDay: number; dueDay: number; color: string } | null;
  onClose: () => void;
  onSave: (data: { name: string; lastDigits: string; limit: number; closingDay: number; dueDay: number; color: string }) => void;
}) {
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-background/80 backdrop-blur-sm">
      <div className="glass-card w-full max-w-md p-6 m-4 border border-border bg-card rounded-2xl shadow-2xl">
