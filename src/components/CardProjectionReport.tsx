...
export function CardProjectionReport() {
  ...
  const hiddenPastCount = useMemo(() => {
    if (hidePast) return current.month;
    let n = 0;
  ...
  const cardProjection = useMemo(() => {
    ...
    for (let off = 0; off <= totalEnd; off++) {
      ...
      months.push({ year: y, month: m, label: `${MONTHS_FULL[m]}/${y}`, key: `${y}-${m}`, isPast });
    }
  ...
  const exportPdf = () => {
    ...
    autoTable(doc, {
      ...
      body: rows.slice(1).map((r) => r.map((c) => (typeof c === "number" ? formatCurrency(c) : String(c)))),
      startY: 50,
          ...
          <div className="flex flex-col items-start sm:items-end gap-2">
            ...
            {state.creditCards.length > 0 && (
              <div className="flex gap-2 flex-wrap">
                <Button variant="outline" size="sm" onClick={() => setHidePast((v) => !v)} title={hidePast ? "Mostrar meses passados" : "Ocultar meses passados"}>
          ...
          <table className="w-full text-xs border-collapse min-w-[900px]">
            <thead>
              <tr className="border-b border-border">
                ...
                {visibleIdx.map((i) => {
                  ...
                  return (
                    <th key={mo.key} className="text-right p-2 font-semibold whitespace-nowrap text-primary-foreground sticky top-0 z-20 bg-primary">
                      <div className="flex items-center justify-end gap-1">
                        <span>{MONTHS_SHORT[mo.month].toLowerCase()}/{String(mo.year).slice(2)}</span>
                        <button
                          type="button"
                          title="Ocultar coluna"
                          onClick={() => setHiddenCols((prev) => { const n = new Set(prev); n.add(mo.key); return n; })}
            ...
            <tbody>
              {cardProjection.cardBlocks.map(({ card, monthly }) => {
                ...
                const rows = Array.from(rowMap.values()).filter((r) => visibleIdx.some((i) => r.perMonth[i] != null));
                  ...
                  <Fragment key={card.id}>
                    {rows.length === 0 ? (
                      <tr key={`${card.id}-empty`} className="border-b border-border/50">
                        ...
                        <td className="p-2 lg:sticky lg:left-[120px] lg:z-20 w-[200px] bg-card text-foreground lg:bg-[color-mix(in_oklab,var(--primary)_88%,white)] lg:text-primary-foreground italic">Sem lançamentos</td>
                        {visibleIdx.map((i) => (<td key={i} className="p-2 bg-primary/5" />))}
                      </tr>
                    ) : (
                      rows.map((row, ri) => (
                        <tr key={`${card.id}-${row.key}`} className="border-b border-border/30 hover:bg-accent/40">
                          ...
                          <td className="p-2 align-middle lg:sticky lg:left-[120px] lg:z-20 w-[200px] bg-card text-foreground lg:bg-primary lg:bg-[color-mix(in_oklab,var(--primary)_88%,white)] lg:text-primary-foreground">
                          ...
                          {visibleIdx.map((i) => {
                            ...
                            const mo = cardProjection.months[i];
                            return (
                              <td key={i} className={`text-right p-0 tabular-nums whitespace-nowrap text-foreground ${mo.isPast ? "bg-primary/5" : "bg-primary/10"}`}>
                                {cell ? (
                                  <button
                                    type="button"
